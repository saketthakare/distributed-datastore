...
def client():
	while True: 
		...
		if choice==1:
			...
			start_time = time.time()
			stub = fileService_pb2_grpc.FileserviceStub(channel)
			# res = stub.FileSearch(fileService_pb2.FileInfo(username="big", filename=name))
			if True:
				response = stub.DownloadFile(fileService_pb2.FileInfo(username="bigtest", filename=name))
